...
def extractStatsValues(stats, numCores):
    ...
    if numCores == 1:
        # Single core - use existing keys without core index, try alternative names
        totalInstructions = int(float(stats.get("simInsts", 0)))
        
        # Try multiple possible names for branches
        totalBranches = int(float(stats.get("system.cpu.numBranches", 
                                         stats.get("system.cpu.branchPred.lookups_0::total", 0))))
        
        totalCycles = int(float(stats.get("system.cpu.numCycles", 0)))
        totalIdleCycles = int(float(stats.get("system.cpu.idleCycles", 0)))
        totalIntAluAccesses = int(float(stats.get("system.cpu.intAluAccesses", 0)))
        totalFpAluAccesses = int(float(stats.get("system.cpu.fpAluAccesses", 0)))
        
        # Try multiple possible names for load/store instructions
        totalLoadInstructions = int(float(stats.get("system.cpu.numLoadInsts", 
                                                  stats.get("system.cpu.commitStats0.numLoadInsts", 0))))
        totalStoreInstructions = int(float(stats.get("system.cpu.numStoreInsts", 
                                                   stats.get("system.cpu.commitStats0.numStoreInsts", 0))))
        
        # Try multiple possible names for branch mispredictions
        branchMispredictions = int(float(stats.get("system.cpu.branchPred.condIncorrect", 
                                                 stats.get("system.cpu.branchPred.mispredicted_0::total", 0))))
        
        # Try multiple possible names for committed instructions
        totalcommittedInstructions = int(float(stats.get("system.cpu.committedInsts", 
                                                       stats.get("system.cpu.commitStats0.numInsts", 0))))
        total_committed_int_instructions = int(float(stats.get("system.cpu.commit.integer", 
                                                             stats.get("system.cpu.commitStats0.numIntInsts", 0))))
        total_committed_fp_instructions = int(float(stats.get("system.cpu.commit.floating", 
                                                            stats.get("system.cpu.commitStats0.numFpInsts", 0))))
        
        totalInstWindowReads = int(float(stats.get("system.cpu.intInstQueueReads", 0)))
        totalInstWindowWrites = int(float(stats.get("system.cpu.intInstQueueWrites", 0)))
        totalInstWindowWakeups = int(float(stats.get("system.cpu.intInstQueueWakeupAccesses", 0)))
        totalFpInstWindowReads = int(float(stats.get("system.cpu.fpInstQueueReads", 0)))
        totalFpInstWindowWrites = int(float(stats.get("system.cpu.fpInstQueueWrites", 0)))
        totalFpInstWindowWakeups = int(float(stats.get("system.cpu.fpInstQueueWakeupAccesses", 0)))
        totalIntRegfileReads = int(float(stats.get("system.cpu.intRegfileReads", 0)))
        totalIntRegfileWrites = int(float(stats.get("system.cpu.intRegfileWrites", 0)))
        totalFunctionCalls = int(float(stats.get("system.cpu.commit.functionCalls", 0)))
        totalIaluAccesses = int(float(stats.get("system.cpu.intAluAccesses", 0)))
        totalFpuAccesses = int(float(stats.get("system.cpu.fpAluAccesses", 0)))
...
def prepareTemplate(outputFile):
    ...
    cache_stats = extractCacheStatsValues(stats, numCores)
    ...
    components_config = {
       ...
       "core0.icache":{
            ...
            "stats": {
                "read_accesses": cache_stats['icache']['read_accesses'],
                "read_misses": cache_stats['icache']['read_misses']
            }
        },
         ...
         "core0.dcache":{
            ...
            "stats": {
                "read_accesses": cache_stats['dcache']['read_accesses'],
                "write_accesses": cache_stats['dcache']['write_accesses'],
                "read_misses": cache_stats['dcache']['read_misses'],
                "write_misses": cache_stats['dcache']['write_misses']
            }
        },
        "core0.BTB":{
        ...
        "L20": {
            ...
            "stats": {
                'read_accesses': cache_stats['l2cache']['read_accesses'],
                'write_accesses': cache_stats['l2cache']['write_accesses'],
                'read_misses': cache_stats['l2cache']['read_misses'],
                'write_misses': cache_stats['l2cache']['write_misses']
            }
        },  
        "L30": {
            ...
            "stats": {
                'read_accesses': cache_stats['l3cache']['read_accesses'],
                'write_accesses': cache_stats['l3cache']['write_accesses'],
                'read_misses': cache_stats['l3cache']['read_misses'],
                'write_misses': cache_stats['l3cache']['write_misses']
            }
        },
...
def extractCacheStatsValues(stats, numCores):
    """Extract and aggregate cache statistics for both single-core and multi-core systems"""
    ...
    if numCores == 1:
        ...
        cache_stats['icache']['read_accesses'] = int(float(stats.get("system.cpu.icache.demandAccesses::total", 0)))
        cache_stats['icache']['read_misses'] = int(float(stats.get("system.cpu.icache.demandMisses::total", 0)))
        
        cache_stats['dcache']['read_accesses'] = int(float(stats.get("system.cpu.dcache.demandAccesses::total", 0)))
        cache_stats['dcache']['write_accesses'] = int(float(stats.get("system.cpu.dcache.WriteReq.accesses::total", 0)))
        cache_stats['dcache']['read_misses'] = int(float(stats.get("system.cpu.dcache.ReadReq.misses::total", 0)))
        cache_stats['dcache']['write_misses'] = int(float(stats.get("system.cpu.dcache.WriteReq.misses::total", 0)))
        
    else:
        # Multi-core - aggregate statistics across all active cores
        import re
        
        # For L1 caches (per-core), aggregate across cores
        for core_id in range(numCores):
            # Check if this core is active (has cycles > 0)
            core_cycles = int(float(stats.get(f"system.cpu{core_id}.numCycles", 0)))
            if core_cycles > 0:
                # Aggregate icache stats
                cache_stats['icache']['read_accesses'] += int(float(stats.get(f"system.cpu{core_id}.icache.demandAccesses::total", 0)))
                cache_stats['icache']['read_misses'] += int(float(stats.get(f"system.cpu{core_id}.icache.demandMisses::total", 0)))
                
                # Aggregate dcache stats
                cache_stats['dcache']['read_accesses'] += int(float(stats.get(f"system.cpu{core_id}.dcache.demandAccesses::total", 0)))
                cache_stats['dcache']['write_accesses'] += int(float(stats.get(f"system.cpu{core_id}.dcache.WriteReq.accesses::total", 0)))
                cache_stats['dcache']['read_misses'] += int(float(stats.get(f"system.cpu{core_id}.dcache.ReadReq.misses::total", 0)))
                cache_stats['dcache']['write_misses'] += int(float(stats.get(f"system.cpu{core_id}.dcache.WriteReq.misses::total", 0)))
    
    # L2 and L3 caches are typically shared, so use the system-level stats
    cache_stats['l2cache']['read_accesses'] = int(float(stats.get("system.l2cache.demandAccesses::total", 0)))
    cache_stats['l2cache']['write_accesses'] = int(float(stats.get("system.l2cache.ReadExReq.accesses::total", 0)))
    cache_stats['l2cache']['read_misses'] = int(float(stats.get("system.l2cache.demandMisses::total", 0)))
    cache_stats['l2cache']['write_misses'] = int(float(stats.get("system.l2cache.ReadExReq.misses::total", 0)))
    
    cache_stats['l3cache']['read_accesses'] = int(float(stats.get("system.l3cache.demandAccesses::total", 0)))
    cache_stats['l3cache']['write_accesses'] = int(float(stats.get("system.l3cache.ReadExReq.accesses::total", 0)))
    cache_stats['l3cache']['read_misses'] = int(float(stats.get("system.l3cache.demandMisses::total", 0)))
    cache_stats['l3cache']['write_misses'] = int(float(stats.get("system.l3cache.ReadExReq.misses::total", 0)))
